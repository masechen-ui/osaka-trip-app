...
export default function ExpensePage() {
  useExchangeRate()   // fetch once on mount
  useExpensesSync()
    ...
    <div className="p-4 pb-6">
      ...
      <div className="bg-card border-[1.5px] border-border rounded-[14px] p-3 mb-3 shadow-card-sm">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <input
            type="number"
            className="w-24 border-[1.5px] border-border rounded-lg px-2 py-1.5 text-sm bg-bg
              focus:outline-none focus:border-accent font-sans text-text"
            value={convAmt}
      ...
      <button
        ...
        className="w-full bg-accent text-white rounded-[14px] py-2.5 text-sm font-medium
          shadow-btn-green active:translate-y-0.5 active:shadow-none transition-all mb-3 border-none cursor-pointer font-sans"
      >
        ＋ 新增記帳
      </button>
      ...
      {showForm && (
        <div className="bg-card border-2 border-border rounded-2xl p-3 mb-3 shadow-card">
          <input
            className="w-full border-[1.5px] border-border rounded-xl p-2 text-sm mb-2 bg-bg
              focus:outline-none focus:border-accent font-sans text-text"
          ...
          <div className="flex gap-2 mb-1">
            <input
              type="number"
              inputMode="decimal"
              className="flex-1 border-[1.5px] border-border rounded-xl p-2 text-sm bg-bg
                focus:outline-none focus:border-accent font-sans text-text"
              placeholder="金額"
              value={form.amount}
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {(Object.keys(CAT_LABELS) as ExpenseCategory[]).map(cat => (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-full border-[1.5px] cursor-pointer font-sans transition-colors
                  ${form.category === cat ? 'bg-accent text-white border-accent' : 'bg-bg border-border text-muted'}`}
              >
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {members.map(m => (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-full border-[1.5px] cursor-pointer font-sans transition-colors
                  ${form.paidBy === m.id ? 'bg-tag-food border-accent-2 text-amber-700' : 'bg-bg border-border text-muted'}`}
              >
                {m.nickname}
              </button>
            ))}
          ...
          <button
            ...
            className="w-full bg-stamp text-white rounded-xl py-2 text-sm font-medium
              border-none cursor-pointer font-sans active:scale-[.98] transition-transform"
          >
            確認記帳
          </button>
        </div>
