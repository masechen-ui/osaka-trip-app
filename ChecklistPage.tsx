...
import { toggleChecklistInFirestore } from '../../hooks/useFirestore'
import { useChecklistSync } from '../../hooks/useFirestore'
...
// Local fallback data — Firestore will override once connected
const DEFAULT_ITEMS: Record<ListTab, { id: string; label: string; isChecked: boolean; badge?: string }[]> = {
  ...
  wish: [
    ...
    { id: 'w6', label: '日本橋買公仔戰利品', isChecked: false },
...
export default function ChecklistPage() {
  ...
  function toggle(id: string) {
    ...
    // Also sync to Firestore if item exists there
    toggleChecklistInFirestore(id, items.find(i => i.id === id)?.isChecked ?? false).catch(() => {})
  }

  function addItem() {
    const label = newItem.trim()
    if (!label) return
    const newEntry = { id: `u-${Date.now()}`, label, isChecked: false }
    setLocalItems(prev => ({ ...prev, [activeTab]: [...prev[activeTab], newEntry] }))
    setNewItem('')
  }
    ...
    <div className="p-4 pb-6">
      ...
      <div className="flex gap-2 mb-3 overflow-x-auto scrollbar-hide pb-1">
        {TABS.map(tab => (
          <button
            ...
            className={`whitespace-nowrap px-3 py-1.5 rounded-xl border-[1.5px] text-xs cursor-pointer
              font-medium transition-all active:scale-95 font-sans
              ${activeTab === tab.id
                ? 'bg-stamp border-stamp text-white'
      ...
      <div className="bg-card border-[1.5px] border-border rounded-[14px] px-4 py-2 shadow-card mb-3">
        {items.map(item => (
          <div
            key={item.id}
            className="flex items-center gap-3 py-2.5 border-b border-border last:border-0 cursor-pointer"
        ...
        <div className="flex gap-2 pt-2">
          <input
            className="flex-1 border-[1.5px] border-border rounded-xl px-3 py-1.5 text-sm bg-bg
          ...
          <button
            onClick={addItem}
            className="bg-accent text-white border-none rounded-xl px-4 text-sm cursor-pointer font-sans
